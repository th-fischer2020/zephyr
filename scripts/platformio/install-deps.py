...
def process_bundled_projects(
    ...
    if state_manifest:
        ...
        for proj in west_manifest["projects"]:
            if "revision" not in proj.keys():
                proj["revision"] = west_manifest.get("defaults", {}).get("revision", "")
            west_dep[proj["name"]] = proj
    ...
    for project_config in west_manifest.get("projects", []):
        ...
        if "revision" not in project_config:
            if west_manifest.get("defaults", {}).get("revision", ""):
                project_config["revision"] = west_manifest.get("defaults", {}).get("revision", "")
