...
def generate_device_api_sections():
    api_sections_file = os.path.join(
        BUILD_DIR,
        "zephyr",
        "include",
        "generated",
        "device-api-sections.ld"
    )

    if os.path.isfile(env.subst(api_sections_file)):
        return
    ...
    cmd = (
        ...
        '"%s"'
        % os.path.join(
            FRAMEWORK_DIR, "scripts", "build", "gen_iter_sections.py"
        ),
        "--alignment", "4",
        "--input",
        os.path.join(
        ...
        os.path.join(
            ...
            "include",
            "generated",
            "device-api-sections.cmake",
        ),
    )

    env.Execute(
        env.VerboseAction(" ".join(cmd), "Generating device API section")
    )
...
def load_west_manifest():
    # Do not use project-based manifest for now because of compatibility issues
    # of PIO and west parser.
    ...
    if os.path.isfile(project_manifest_path):
        print("Info: Using a project-based dependency manifest file.")
...
def get_zephyr_venv_dir():
    ...
    # PlatformIO venv
    raw_zephyr_package_version = platform.get_package_version("framework-zephyr")

    if 'sha' in raw_zephyr_package_version:
        version_end_index = raw_zephyr_package_version.index('+sha')
        zephyr_package_version = raw_zephyr_package_version[:version_end_index]
    else:
        zephyr_package_version = raw_zephyr_package_version

    zephyr_version = version.get_original_version(zephyr_package_version)
